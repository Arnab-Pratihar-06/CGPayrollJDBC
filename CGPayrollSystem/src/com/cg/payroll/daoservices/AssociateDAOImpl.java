	...
	public Associate save(Associate associate) {
		try {
			...
			PreparedStatement pstmt1=
					con.prepareStatement("insert into Associate(associateId,yearlyInvestmentUnder80C,firstName, lastName,department,designation,pancard,emailId)values(ASSOCIATE_ID_SEQ.NEXTVAL,?,?,?,?,?,?,?)");
	...
	public Associate findOne(int associateId) {
		try {
			...
			if(associateRs.next()) {
				...
				Salary salary=new Salary(basicSalary,hra,conveyenceAllowance,otherAllowance,personalAllowance,monthlyTax,epf,companyPf,netSalary);
